 ...
 * Stores -> Config -> eTechFlow -> Advanced Product Reviews -> Payment Settings,
...
class Checkout extends Action
{
    ...
    private const XML_STRIPE_CURR   = 'etechflow_reviews/payment/stripe_currency';
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$stripeKey || str_starts_with($stripeKey, '****')) {
            $this->messageManager->addErrorMessage(
                __('Stripe Secret Key is not configured. Go to Stores -> Config -> eTechFlow -> Advanced Product Reviews -> Payment Settings.')
            );
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('etechflow_reviews/license/gate');
        }

        // Price + name come AUTHORITATIVELY from the portal (admin-controlled),
        // so the merchant can't tamper with the amount.
        ...
        $successUrl = $this->getUrl('etechflow_reviews/license/activated')
            . '?session_id={CHECKOUT_SESSION_ID}&plan=' . urlencode($plan)
            . '&domain=' . urlencode($domain) . '&name=' . urlencode($name) . '&email=' . urlencode($email);
        $cancelUrl  = $this->getUrl('etechflow_reviews/license/gate');

        $postData = http_build_query([
            'payment_method_types[0]'                              => 'card',
            'line_items[0][price_data][currency]'                  => $currency,
            'line_items[0][price_data][product_data][name]'        => $planInfo['name'] . ' — ETechFlow',
            'line_items[0][price_data][product_data][description]' => 'Advanced Product Reviews module for ' . $domain,
     ...
     * /license/plans endpoint. Returns null if the plan isn't offered.
     *
     * @param string $slug
     * @param string $domain
