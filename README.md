...
- **Themes:** Luma and Hyvä (Hyvä support is **built in** — no separate module needed), and headless storefronts (PWA Studio / custom)
...
- **Hyvä** UI is bundled in: a Tailwind + Alpine PDP component renders the whole reviews experience from the GraphQL API, and activates automatically when a Hyvä theme is active
...
> **Hyvä stores:** no extra package is needed — the Hyvä reviews UI is bundled
> in this module and turns on automatically when a Hyvä theme is active. (Install
> the Hyvä theme itself, `hyva-themes/magento2-theme-module`, as usual.)
